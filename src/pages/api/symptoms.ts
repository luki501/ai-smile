...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    const { data, count } = await symptomService.getSymptoms(session.user.id, validationResult.data);
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    const symptomService = new SymptomService(supabase);
    const newSymptom = await symptomService.createSymptom(session.user.id, validationResult.data);
