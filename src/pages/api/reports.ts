...
export const POST: APIRoute = async (context) => {
  ...
  if (!validation.success) {
    const errorMessage =
      validation.error.errors[0]?.message || "Invalid period_type. Must be one of: week, month, quarter";
    return new Response(
      JSON.stringify({
...
export const GET: APIRoute = async (context) => {
  ...
  try {
    const result = await fetchReports(context.locals.supabase, userId, offset, limit, period_type);
