...
import type { CreateSymptomCommand, SymptomDetailsDto, SymptomDto, UpdateSymptomCommand } from "@/types";
...
export class SymptomService {
  ...
  async createSymptom(userId: string, symptomData: CreateSymptomCommand): Promise<SymptomDetailsDto> {
  ...
  async getSymptoms(
    ...
    const { limit, offset, body_part, occurred_at_gte, occurred_at_lte, symptom_type } = filters;

    let query = this.supabase.from("symptoms").select("*", { count: "exact" }).eq("user_id", userId);
  ...
  async getSymptomById(symptomId: number, userId: string): Promise<SymptomDetailsDto | null> {
    const { data, error } = await this.supabase
      .from("symptoms")
      .select("*")
      .eq("id", symptomId)
